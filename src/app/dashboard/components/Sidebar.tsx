...
export default function Sidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      {/* Mobile Drawer */}
      <div className={`fixed inset-0 bg-black/40 z-40 transition-opacity ${open ? 'opacity-100' : 'opacity-0 pointer-events-none'} md:hidden`} onClick={onClose} />

      <aside
        className={`
    bg-base-200 border-r border-base-300 p-4 h-full w-64 z-50
